...
def Genome_wide_analysis(CDS_file):
	...
	if model_fix == "find":
		...
		while i < len(Options):
			...
			while j < len(Options):
				...
				while k < len(Options):
					...
					while l < len(Options):
						opt_4 = Options[l]
						model = opt+"_"+opt_2+"_"+opt_3+"_"+opt_4
						number_para = 4
						res_model, log_likelihood_model, r2_model = get_math_function(model, 0, 0, 0, 0)
						AIC_model = (2*number_para)-(2*(log_likelihood_model))
						BIC = log_number_data_points*number_para - (2*log_likelihood_model)
		...
		if 'Bp' in model_fix:
			...
			print "Please input the specified model using the options Mb Sc St Bp separated by _\neg. Mb_Sc_St or Mb_St or Mb etc."
	...
	print "Results Summary:\nLn_L = %d\nAIC =\t%d\nR2 =\t%s (excluding amino acids with only one codon)\nMb =\t%s\nSc =\t%s\nSt =\t%s\nBp =\t%s\nModel = %s\n" %(best_log_likelihood, AIC, str(best_r2), str(best_res[0]), str(best_res[1]), str(best_res[2]), str(best_res[3]), model_used)
	return best, best_res[0], best_res[1], best_res[2], best_res[3]
...
def per_gene_analysis(best_model):
	...
	f1.write("Accession\tLog_likelihood\tMb\tSc\tSt\tBp")
	try:
	...
	for acc in cds_filtered:
		...
		f1.write("\n"+acc+"\t"+str(likelihood_model)+"\t"+str(res_model[0])+"\t"+str(res_model[1])+"\t"+str(res_model[2])+"\t"+str(res_model[3]))
	f1.close()

def get_math_function(model_type, start_Mb, start_Sc, start_St, start_Bp):
	equati = write_equation(model_type, Mb_to_use, Sc_to_use, St_to_use, Bp_to_use)
	def equation_real(x):
		Mb, Sc, St, Bp = x
		try:
			return eval(equati)
			...
			return 10000000000000000000
	x0 = np.asarray((start_Mb, start_Sc, start_St, start_Bp))
	...
	if 'Bp' in model_type:
		if Bp_to_use == 'Bp':
			result_parameters[3] = round(result_parameters[3], 4)
		else:
			result_parameters[3] = Bp_to_use
	else:
...
def get_r2_value(x, model_type):
	...
	for codon in relative_codon_use:
		probability_codon = get_probability_codon(codon, model_type, Mb, Sc, St, Bp)
...
def get_probability_codon(input_codon, model_type, Mb, Sc, St, Bp):
	...
	for parameter in parameters:
		number_parameters -= 1
		for syn in proteins[prot]:
			additional = get_upper_equation(syn, parameter, Mb, Sc, St, Bp, 'for_probability', 'off')
...
def get_upper_equation(codon, parameter, Mb, Sc, St, Bp, equation_type, shuffle_type):
	...
	if parameter == 'Mb':
		...
		if shuffle_type == 'off':
			BP_selection = '%.4f'%(BP_value[codon])
		else:
			BP_selection = '%.4f'%(BP_shuffled[codon])
		return "(math.exp("+str(Bp)+"*"+BP_selection+"))*"
	else:
		print "%s is not a recognised parameter\nOnly Mb, Sc, St and Bp are recognised parameters\n\n" %parameter

def get_p_value(shuffle_type, real_likelihood, accuracy, Mb_in, Ns_in, Te_in, Bp_in): #
	counter = 1
	better_shuffled = 0
	while counter <= accuracy:
		...
		def equation_shuffle(x):
			...
			try:
				...
				return 10000000000000000000
		x0 = np.asarray((Mb_in, Ns_in, Te_in, Bp_in))
...
def write_equation(model_type, Mb, Sc, St, Bp):
	...
	if 'shuffle' in model_type:
		...
		Bp_shuffled = using_shuffle(BP_value)
	...
	for parameter in parameters:
		...
		if parameter == 'shuffle':
			...
			for codon in alphabetic_codons:
				if shuffle == 'off':
					additional = get_upper_equation(codon, parameter, Mb, Sc, St, Bp, 'for_real', 'off')
				else:
					additional = get_upper_equation(codon, parameter, Mb, Sc, St, Bp, 'for_shuffle', 'on')
...
def get_BP_values(input):
	global BP_value
	BP_value = {}
	with open(input) as BPvalues:
		for line in BPvalues:
			line = str(line.rstrip().strip())
			bits = line.split()
			codon = bits[0]
			if codon in codon_trans_standard:
				BP_value[codon] = float(bits[1])
			else:
				if codon != "Codon":
					print "Your BP value contains %s which is not a codon\n" %(codon)
...
St_to_use = 'St'
Bp_to_use = 'Bp'
if "-bp_file" in sys.argv:
	base_pair_index = command_line.index("-bp_file") + 1
	base_pair_file = sys.argv[base_pair_index]
if "-Bp" in sys.argv:
	Bp_index = command_line.index("-Bp") + 1
	Bp_specified = sys.argv[Bp_index]
	if Bp_specified != "gw":
		Bp_specified = float(Bp_specified)
		Bp_to_use = Bp_specified
else:
	Bp_specified = "No"
if "-Mb" in sys.argv:
...
model_to_use, Mb_gw, Sc_gw, St_gw, Bp_gw= Genome_wide_analysis(CDS_file)
...
if Bp_specified == "gw":
	Bp_to_use = Bp_gw
	print "\nBp fixed to the genome-wide Bp (%d) for individual gene analysis." %(Bp_gw)
